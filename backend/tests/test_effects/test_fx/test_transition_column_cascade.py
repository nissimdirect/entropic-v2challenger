"""Tests for fx.transition_column_cascade — vertical columns fill left→right."""

import numpy as np
import pytest

from effects.fx.transition_column_cascade import EFFECT_ID, PARAMS, apply
...
def test_effect_id():
    assert EFFECT_ID == "fx.transition_column_cascade"
...
def test_param_clamping_at_trust_boundary():
    f = _frame()
    key = _frame(seed=7)
    bad = {"progress": 99.0, "edge_softness": -5.0, "_sidechain_frame": key}
    out, _ = apply(f, bad, None, **KW)
    assert out.dtype == np.uint8
    assert not np.isnan(out.astype(np.float64)).any()


def test_sidechain_rgba_key_frame_supported():
    """`_sidechain_frame` may itself be RGBA (4-channel) — must not error."""
    f = _frame()
    key = _frame(seed=7)  # already RGBA
    out, _ = apply(f, {"progress": 0.5, "_sidechain_frame": key}, None, **KW)
    assert out.shape == f.shape


def test_sidechain_mismatched_resolution_resizes():
    """Key frame at a different resolution must be resized to match main."""
    f = _frame(h=64, w=64)
...
def test_progress_monotonic_increases_blue_fraction():
    """More progress must move mean output color monotonically toward the key."""
