...
def test_real_backend_loader_seam_does_not_load_models(backend):
    ...
    loader = make_loader("dinov2", backend=backend)
    assert loader.name == "dinov2"
    # cold_load_seconds is None until first real encode (PR #4)
    assert loader.cold_load_seconds is None
    # encode() raises NotImplementedError in PR #3
