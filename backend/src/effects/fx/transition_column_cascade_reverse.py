"""Transition: Column Cascade Reverse — vertical columns fill right→left.

LAYER-TRANSITIONS.md #2 (Geometric Reveals). Same mechanism as
`transition_column_cascade` (see that module's docstring and
`docs/plans/transitions-pattern.md`) — only the sweep-axis direction differs.
"""
...
def apply(
    ...
    *,
    ...
    """Column Cascade Reverse: reveal layer B in vertical columns, right→left."""
    ...
    pos = 1.0 - (x / max(1, w - 1))  # 0 (right) .. 1 (left) -> reveals right-to-left
