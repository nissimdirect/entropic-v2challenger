...
describe('MacroKnob logic', () => {
  it('normalizes value to percentage', () => {
    const macro: MacroMapping = { label: 'Test', effectId: 'fx-1', paramKey: 'amount', min: 0, max: 1 }
    const value = 0.5
  ...
  it('handles non-zero min', () => {
    const macro: MacroMapping = { label: 'Test', effectId: 'fx-1', paramKey: 'freq', min: 100, max: 500 }
    const value = 300
    const normalized = (value - macro.min) / (macro.max - macro.min)
    expect(Math.round(normalized * 100)).toBe(50)
  })
  ...
  it('clamps at boundaries', () => {
    const macro: MacroMapping = { label: 'Test', effectId: 'fx-1', paramKey: 'x', min: 0, max: 10 }
