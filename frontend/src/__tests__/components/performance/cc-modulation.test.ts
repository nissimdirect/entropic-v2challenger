...
describe('applyCCModulations', () => {
  it('returns original chain when no modRoutes', () => {
  ...
  it('applies single CC mapping (absolute set)', () => {
    const chain = [makeEffect('e1', { amount: 0.5 })];
    const modRoutes: CCMapping[] = [{ cc: 1, effectId: 'e1', paramKey: 'amount' }];
    ...
    const result = applyCCModulations(chain, modRoutes, ccValues, registry);
    expect(result[0].parameters.amount).toBeCloseTo(0.75, 4);
  });

  it('scales CC to param range', () => {
    const chain = [makeEffect('e1', { freq: 200 })];
    const modRoutes: CCMapping[] = [{ cc: 2, effectId: 'e1', paramKey: 'freq' }];
  ...
  it('skips CC without value', () => {
    const chain = [makeEffect('e1', { amount: 0.5 })];
    const modRoutes: CCMapping[] = [{ cc: 99, effectId: 'e1', paramKey: 'amount' }];
  ...
  it('does not mutate original chain', () => {
    ...
    applyCCModulations(chain, modRoutes, ccValues);
