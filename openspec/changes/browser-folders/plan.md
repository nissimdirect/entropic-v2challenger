...
| `frontend/src/renderer/App.tsx` | `sidebarTab` local `useState` (line 455) + render switch (line 3637-3691, three-button tab bar + conditional render of `EffectBrowser`/`InstrumentsBrowser`/`PresetBrowser`); `toggle_sidebar` shortcut → `useLayoutStore.toggleSidebar()` (line 695) | Delete `sidebarTab` state + the 3-button bar (OD-1 default). Replace the conditional render block with the new tree/rail root component, mounted once. |
...
| `frontend/src/shared/types.ts:529-541` | `EffectInfo { id, name, category, params, fieldParams? }` | No change — the tree consumes this shape as-is. |
...
| everything else (26 remaining strings, incl. `composite` — OD-7) | EFFECTS | one EFFECTS subfolder per remaining category string, alphabetical, mirroring today's `EffectBrowser.tsx:349-352` `categories` computation (`Array.from(new Set(...)).sort()`) verbatim — do not hand-curate a subfolder list, derive it |
...
2. `entropic-effect-browser-expanded` (`EffectBrowser.tsx:226`) → `Set<string>` of EXPANDED category
   names, defaulting to **all categories expanded** on first run with no stored value
   (`EffectBrowser.tsx:333-337`: `hasStored ? value : new Set(registry.map(e => e.category))`).
...
Current enum (`DragPayload.kind`, `EffectBrowser.tsx:48`): `'fx' | 'op' | 'composite' | 'instruments' |
'operator'`. Validated against `['fx', 'op', 'composite', 'instruments', 'operator']` at
`EffectBrowser.tsx:77` and namespace-regexed at line 80 (`/^(builtin:|user:)/`).
...
- GENERATORS' one leaf (`fx.moire`) and UTILITIES' five leaves are ordinary registry `EffectInfo`
  ...
  `handleDragStart`, `EffectBrowser.tsx:435-451`, which only branches on `info.category === 'composite'`
...
| **P9 — Outer `sidebarTab` + inner `activeTab` deletion (OD-1)** | `App.tsx:455,3637-3691`, `browser.ts:8-9,18-20,82-86` | MED (touches the mount point every other packet's UI now lives inside) | full click-through UAT: every entry reachable today via `sidebarTab='effects'/'presets'/'instruments'` AND via `EffectBrowser.activeTab='fx'/'op'/'composite'/'tool'/'instruments'` is reachable from exactly one place in the new tree — a checklist derived from this plan's §1 table, zero duplicate entry points remaining (closes the duplicate-INSTRUMENTS finding in proposal OD-1) |
