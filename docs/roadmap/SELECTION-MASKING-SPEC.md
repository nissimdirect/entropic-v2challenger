...
> **ALL DECIDED 2026-06-12 (user: "your recs are good"):** D1 procedural-only (Freeze/Flatten = the bake gesture) · D2 local RVM only, cloud BiRefNet stays out until explicit per-project opt-in · D3 WebM/VP9-alpha INCLUDED in MK.10 · D4 flatten-to-surface-0 in v1, checkerboard ships as a view toggle with MK.13 view modes · D5 matte cache = own 128 MiB pool (SG-8 degrades independently) · D6 RVM torch = separate venv subprocess, sidecar stays lean · D7 keyboard-only polygon/paint accepted as v1 a11y gaps with filed follow-up. Packets read these as LOCKED.
