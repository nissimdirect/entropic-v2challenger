...
- **ACCEPTANCE GATES:** full-library run completes (crashes isolated, not fatal) · contact sheets exist for ≥ registry count minus explicitly skipped (list skips with reasons) · known-bad detection: v1 grid_moire flagged · backend smoke (`pytest backend/tests -x -q -m "not slow"` or project norm) green · **visual artifact:** the codec_archaeology contact sheet itself attached to PR.
