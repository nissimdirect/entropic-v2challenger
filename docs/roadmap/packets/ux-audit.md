...
- **TEST PLAN:** `cd frontend && npx --no vitest run` (no regressions vs precondition snapshot) · `npx playwright test` smoke · `grep -REho '#1a1a1a|#4ade80([^0-9a-fA-F]|$)' src/renderer/styles | wc -l` → 0 in styles (POSIX ERE — plain `grep -E` has no lookahead; `rg -o '#4ade80([^0-9a-fA-F]|$)'` is the equivalent if ripgrep is preferred) · visual: launch `npm start`, screenshot timeline/browser/dialogs, compare against pre-branch screenshots — pixel-identical if palette decision deferred, intentional-diff-only otherwise.
